(function (w) {
     ...
     * @param callback 回调函数组成的对象  {start:fn, move: fn, touchend:fn end:fn}
     * */
    w.touchscroll = function (wrap, inner, scrollBar, callback) {


        wrap.timeId = null;   // 定义标记

        // 滚动条相关设置
        if (scrollBar) {
            wrap.scale2 = wrap.clientHeight / inner.offsetHeight;  // 计算比例
            scrollBar.style.height = wrap.clientHeight *wrap.scale2 + 'px';   // 设置滚动条高度
        ...
        wrap.addEventListener('touchmove', function (event) {
            ...
            // 滚动条位置
            if (scrollBar) {
                var scrollBarY = -translateY * wrap.scale2;
        ...
        wrap.addEventListener('touchend', function () {
            ...
            // 如果存在回调函数
            if (callback && typeof callback['touchend'] === 'function') {
            callback['touchend']();
            }
        ...
        function tweenMove(target, duration, type) {
            ...
            // 开启定时
            wrap.timeId = setInterval(function () {
                t += delay;  // 开始时间，每次都要变化
                ...
                if (scrollBar) {
                    // 滚动条位置
                    var scrollBarY = -currVal * wrap.scale2;
                    transformCss(scrollBar, 'translateY', scrollBarY);  // 设置滚动条位置
                }
                ...
                // 持续时间到了
                if (t >= d) {
                    clearInterval(wrap.timeId);  //停止定时
                    // 有滚动条
                    if (scrollBar) {
